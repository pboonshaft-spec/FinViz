...
export function useDataProcessor() {
  const processData = useCallback((filesData) => {
    const logs = [];
    ...
    allData.forEach((row, idx) => {
      ...
      if (!date || isNaN(date)) {
        if (idx < 5) log(`Warning: Row ${idx}: Invalid date "${row[columns.dateCol]}"`);
        return;
      }
      ...
      if (columns.debitCol && columns.creditCol) {
        ...
        if (isNaN(amount)) {
          if (idx < 5) log(`Warning: Row ${idx}: Invalid amount "${row[columns.amountCol]}"`);
          return;
        }
